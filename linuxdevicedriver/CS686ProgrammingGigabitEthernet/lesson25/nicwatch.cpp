...
char	*pktype[] = { "incoming", "broadcast", "multicast", "otherhost",
...
{
	printf("./a.out interface -c packet_limit pktype\n");
	// allow user to modify the default number of packets to accept
	for (int i = 1; i+1 < argc; i++)
		if ( strncmp( argv[ i ], "-c", 3 ) == 0 )
			packet_limit = atoi( argv[ i+1 ] );

	display_type=atoi(argv[4]);
	printf( "monitor (%s) packets ", pktype[ 7 & display_type ] );
