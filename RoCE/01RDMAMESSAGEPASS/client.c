...
int main(int argc, char **argv)
{
    ...
    if (argc != 4)
        die("usage: client <server-address> <server-port> <pingpong times>");
    ...
    freeaddrinfo(addr);
    num_completion=strtol(argv[3],NULL,10);
    printf("we are going to communicate for %d  times and say goodbye\n",num_completion);
...
void post_send(struct rdma_cm_id *id)
{
    ...
    snprintf(conn->send_region, BUFFER_SIZE, "%dth message from active/client side with pid %d",num_completion, getpid());
...
void on_completion(struct ibv_wc *wc)
{
    ...
    printf("status code=%d ",wc->status);
...
int on_connection(struct rdma_cm_id *id)
{
    ...
    printf("connected. performing %dth message exchange  posting send...\n",num_completion);
    snprintf(conn->send_region, BUFFER_SIZE, "%dth message from active/client side with pid %d",num_completion, getpid());
